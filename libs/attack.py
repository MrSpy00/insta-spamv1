...
def report_profile_attack(username, proxy):
    ...
    try:
        ...
        print_error("Bağlantı hatası! ~FacebookRequestsError~")
        return

    if (res.status_code != 200):
        print_error("Bağlantı hatası! (STATUS CODE:", res.status_code, ")")
        return
    ...
    try:
        ...
        print_error("Bağlantı hatası! ~CookieParsingError~")
        return
    ...
    try:
        ...
        print_error("Bağlantı hatası! ~InstagramRequestsError~")
        return
    ...
    if ('["LSD",[],{"token":"' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorLSD~")
        return

    if ('"__spin_r":' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorSpinR~")
        return

    if ('"__spin_b":' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorSpinB~")
        return

    if ('"__spin_t":' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorSpinT~")
        return

    if ('"server_revision":' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorRev~")
        return

    if ('"hsi":' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorHsi~")
        return
    ...
    try:
        ...
        print_error("Bağlantı hatası! ~FormRequestsError~")
        return
...
def report_video_attack(video_url, proxy):
    ...
    try:
        ...
        print_error("Bağlantı hatası! (FacebookRequestsError)", e)
        return
    ...
    if ('["_js_datr","' not in res.text):
        print_error("Bağlantı hatası! ~CookieErrorJSDatr~")
        return
    ...
    if ("datr" not in res.cookies.get_dict()):
        print_error("Bağlantı hatası! ~CookieErrorDatr~")
        return
    ...
    print_success("Şikayet başarılı!")
