...
def video_attack(proxies):
    video_url = ask_question("Şikayet edilmesini istediğiniz videonun linkini giriniz!")
    ...
    print("")
    print_status("Video Şikayeti Başlıyor!\n")
...
def profile_attack(proxies):
    username = ask_question("Şikayet edilecek kişinin kullanıcı adını giriniz..")
    print(Style.RESET_ALL)
    if (len(proxies) == 0):
        for k in range(5):
    ...
    print("")
    print_status("Profil şikayeti bağlıyor!\n")
...
def main():
    ...
    if (ret == "E" or ret == "e"):
        ...
        if (ret == "E" or ret == "e"):
            print_status("Proxyler toplanıyor! Biraz sürebilir..\n")
            proxies = find_proxies()
        elif (ret == "H" or ret == "h"):
            print_status("Dosyada 50'den fazla proxy bulunamaz!")
            file_path = ask_question("Proxy listenizin yolunu giriniz")
            proxies = parse_proxy_file(file_path)
        else:
            print_error("Cevap anlaşılamadı, tool kapanıyor!")
            exit()
    ...
    print("")
    print_status("Profil Şikayeti ~ (1)")
    print_status("Video Şikayeti ~ (2)")
    report_choice = ask_question("Şikayet numarasını giriniz!")
    print("")
